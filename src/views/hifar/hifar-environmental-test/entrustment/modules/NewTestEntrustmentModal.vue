        ...
        <div class="item-wrapper">
          ...
          <div class="item-wrapper-content">
            <div style="margin-top:20px">
              <template v-if="entrustType == 1 && tableData.length === 0">
                <a-button
                  icon='plus'
                  size='small'
...
export default {
      ...
      return [
        ...
        },
        {
          ...
          change: (val, option) => {
            ...
            this.entrustModel = Object.assign({}, this.entrustModel, {entrustType: val, isExternalManage: '0'})
            switch (val) {
              case '1':
                this.internalAdd()
                break
        ...
        //   key: 'testType',
        //   formType: 'dict',
        //   dictCode: 'hf_entrust_test_type',
        //   validate: {
        //     rules: [{required: true, message: '请选择试验类型'}]
        //   },
        //   change: (val, option) => {
        //     this.$refs.entrustFrom.form.setFieldsValue({testType: option.title})
        //     this.entrustModel.testType = val
        //   }
        // },
        {
          title: '报告交付进度要求',
          key: 'progressRequire',
          formType: 'dict',
          dictCode: "hf_report_progress_require",
          hidden: this.entrustType === '1',
        },
        {
          title: '结算方式',
          key: 'settlementType',
          formType: 'dict',
          dictCode: "hf_settlement_type",
          hidden: this.entrustType === '1',
        },
        {
          title: this.entrustType === '1' ? '委托方批准人' : '委托方代表',
          key: 'entrustApproval',
          formType: 'input',
          validate: {
            rules: [{required: true, message: `请输入${this.entrustType === '1' ? '委托方批准人' : '委托方代表'}`}]
          }
        },
        {
          title: '是否允许对试品进行拍照',
          key: 'isPhotographByPiece',
          formType: 'radio',
          radioType: 'radioButton',
          defaultValue: '2',
          hidden: this.entrustType === '1',
  ...
  methods: {
    ...
    handleAdd(record) {
      this.entrustModel = {
        linkName: record.idName,
        linkMobile: record.mobile,
        isReport: 1,
        isExternalManage: '0',
        entrustTime: moment(),
        attachIds: [],
    ...
    internalAdd() {
      ...
      this.$set(this.entrustModel, 'custName', userInfo.deptName)
      this.$set(this.entrustModel, 'custId', 'default')
      this.$set(this.entrustModel, 'linkName', userInfo.idName)
      this.$set(this.entrustModel, 'linkMobile', userInfo.mobile)
      this.$set(this.entrustModel, 'secretLevelCode', 1)
      this.$set(this.entrustModel, 'isReport', 2)
    },
    // 委托单类型 外部
    outerAdd() {
      this.entrustModel.custId = ''
      this.entrustModel.custName = ''
      this.$set(this.entrustModel, 'secretLevelCode', 1)
      this.$set(this.entrustModel, 'isReport', 1)
