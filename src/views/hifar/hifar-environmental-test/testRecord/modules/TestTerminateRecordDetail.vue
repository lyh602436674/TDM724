<!--
 * @Author: 赵峰
 * @Date: 2021-11-02 17:46:03
 * @LastEditTime: 2021-11-10 14:54:03
 * @LastEditors: 赵峰
 * @Descripttion: 试验终止记录详情
 * @FilePath: \hifar-platform-client\src\views\hifar\hifar-environmental-test\testRecord\modules\TestTerminateRecordDetail.vue
-->
<template>
  <h-modal
    inner
  ...
  >
    ...
    <h-card title="试验信息" fixed :showCollapse="true">
      <h-desc slot="content" size="small" labelWidth="120px">
        <h-desc-item v-for="(item, index) in testData" :span="item.span ? item.span : 1" :key="index" :label="item.title">
          {{
            item.isTime
    ...
    <h-card title="终止信息" fixed :showCollapse="true">
      <h-desc slot="content" size="small" labelWidth="120px">
        ...
        <h-desc-item label="终止说明"> {{ detailData.forceEndDesc || '--' }}</h-desc-item>
      </h-desc>
    </h-card>
